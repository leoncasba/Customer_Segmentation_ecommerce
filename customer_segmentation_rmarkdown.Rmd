...
scale2 <- apply(RFM_2[, -1], 2, scale, center = TRUE, scale = TRUE)
...
```{r}
library(clusterSim)
test4 = kmeans(scaled_values, centers=4, nstart=20)
test2 = kmeans(scaled_values, centers=2, nstart=20)
test3 = kmeans(scaled_values, centers=3, nstart=20)
test5 = kmeans(scaled_values, centers=5, nstart=20)
test6 = kmeans(scaled_values, centers=6, nstart=20)
#print(test$cluster)


k2 <- as_tibble(scaled_values) %>% mutate(cluster=test2$cluster)
k3 <- as_tibble(scaled_values)  %>% mutate(cluster=test3$cluster)
k4 <- as_tibble(scaled_values)  %>% mutate(cluster=test4$cluster)
k5 <- as_tibble(scaled_values) %>% mutate(cluster=test5$cluster)
k6 <- as_tibble(scaled_values) %>% mutate(cluster=test6$cluster)
...
plot(scale2,
     col = test$cluster,
     main = paste("K-means clustering of Congos' customers with", 4, "clusters"),
     xlab = "", ylab = "")

i2 <- index.DB(k2, k2$cluster, d=NULL, centrotypes="centroids", p=2, q=2)
i3 <- index.DB(k3, k3$cluster, d=NULL, centrotypes="centroids", p=2, q=2)
i4 <- index.DB(k4, k4$cluster, d=NULL, centrotypes="centroids", p=2, q=2)
i5 <- index.DB(k5, k5$cluster, d=NULL, centrotypes="centroids", p=2, q=2)
i6 <- index.DB(k6, k5$cluster, d=NULL, centrotypes="centroids", p=2, q=2)

rbind(i2$DB, i3$DB, i4$DB, i5$DB, i6$DB)
```
...
pam_k2 <- pam(scaled_values, k = 4)

# Plot the silhouette visual for the pam_k2 model
plot(silhouette(pam_k2))

# Generate a k-means model using the pam() function with a k = 3
pam_k3 <- pam(scaled_values, k = 3)
