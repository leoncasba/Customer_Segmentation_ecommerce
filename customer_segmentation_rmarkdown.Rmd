...
From previous RFM analysis we assume there about 5 group of clusters, however, there is a big amount of customers who remains in the "Other" categories. 
...
Since K-means algorithm works with euclidean distances we need to escalate the data. It means transforming data in order the mean is equal 1 and the standard deviation equal 1.
...
### Selecting the optimum K (number of clusters) 
...
Idea behind the elbow method is to identify the value of k where the score begins to decrease most rapidly before the curve reached a plateau. In the figure below, we can see it seems is around 3 or 4, however, it can get confusing and that is the reason we will explore further on the next measures.

```{r, , fig.align='center', fig.height=5, fig.width=9,fig.cap = '**Figure 7.** Elbow method for optimum K.', message=FALSE, warning=FALSE}
...
When using the the silhouette analysis we are normally looking for a value of k that provides the high average silhouette score, however, in this case the figure shows is k=2. However, I will follow to logic presented by the previous two metrics which make more sense on the context of the problem I'm solving.
...
```{r, fig.align='center', fig.height=7, fig.width=14,fig.cap = '**Figure 9.** Cluster plot.', message=FALSE, warning=FALSE}
set.seed(1995)
...
This is the interpretation provided to the k-means clusters:
  ...
  * **Cluster 2** has 21.16% customers. It belongs to "Alomost Lost". They purchase recently (R=2). However they do not purchase frequently and do not spent a lot. (R=2, F=3, M=3)
