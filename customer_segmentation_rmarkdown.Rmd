...
q1 <- data_prepared %>% group_by(year(TransactionDate), month(TransactionDate)) %>% summarize(total=sum(Unit_Product_Price*Quantity_Bought)) %>% rename(Year='year(TransactionDate)', Month='month(TransactionDate)', Total=total)
...
q3 <- data_prepared %>% group_by(Product_Name) %>% summarise(Total_Revenue_by_Product=sum(Unit_Product_Price*Quantity_Bought)/1000) %>% arrange(desc(Total_Revenue_by_Product)) %>% top_n(10)

ggplot(q3, aes(x=reorder(factor(Product_Name),Total_Revenue_by_Product), y=Total_Revenue_by_Product, fill = ifelse(Product_Name == c("Dryer Machine", "King Size Bed", "Individual Bed"), "Highlighted", "Normal") ))+geom_col()+theme_classic()+coord_flip()+theme(legend.position = "none", axis.title.y = element_blank())+
  labs(y="Total revenue by product ($ thousands)")
```
...
RFM analysis allows to segment customers by the frequency and value of purchases and identify those customers who spend the most money.
  ...
  * **Recency** means how long it’s been since a customer bought something from us.
  * **Frequency** represents how often a customer buys from us.
  * **Monetary value** is the total value of purchases a customer has made.
...
recency <- data_prepared %>% group_by(Customer_Code) %>% summarise(Recency=(NOW-max(TransactionDate)))
```
